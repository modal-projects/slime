...
def _agentic_metrics(samples) -> dict:
    stats = [s.metadata["agentic"] for s in samples if isinstance(getattr(s, "metadata", None), dict) and "agentic" in s.metadata]
    if not stats:
        return {}
...
def _async_metrics(samples, now) -> dict:
    ...
    ages = [now - s.metadata["agentic"]["gen_timestamp"] for s in samples if isinstance(getattr(s, "metadata", None), dict) and s.metadata.get("agentic", {}).get("gen_timestamp")]
    if ages:
...
def log_rollout_data(rollout_id, args, samples, rollout_extra_metrics, rollout_time) -> bool:
    ...
    try:
        ...
        if samples:
            extra["agentic/removed_frac"] = float(np.mean([1.0 if getattr(sample, "remove_sample", False) else 0.0 for sample in samples]))
        if extra:
            logger.info("agentic %s: %s", rollout_id, {k: round(v, 3) for k, v in extra.items() if isinstance(v, (int, float))})
