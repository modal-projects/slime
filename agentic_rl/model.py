...
class RecordingModel:
    ...
    def _parse_actions(self, segment: str, finish: str) -> list[dict]:
        ...
        if len(calls) != 1:
            raise self._tool_call_format_error(len(calls), segment, f"expected exactly one tool call, found {len(calls)}", finish)
        fn = _FUNCTION_RE.search(calls[0])
        if not fn:
            raise self._tool_call_format_error(1, segment, "tool call had no <function=...> block", finish)
        if fn.group(1) != "bash":
            raise self._tool_call_format_error(1, segment, f"called tool {fn.group(1)!r}; only 'bash' is available", finish)
        cmd_match = _PARAM_CMD_RE.search(fn.group(2))
        cmd = cmd_match.group(1) if cmd_match else None
        if not isinstance(cmd, str) or not cmd.strip():
            raise self._tool_call_format_error(1, segment, "the bash tool call had no <parameter=command> value", finish)
        return [{"command": cmd}]
    ...
    def format_observation_messages(self, message: dict, outputs: list[dict], template_vars: dict | None = None):
        ...
        return format_observation_messages(outputs, observation_template=self.observation_template, template_vars=template_vars)
