...
commands):
    ...
    ROLLOUT_MODE=eval FRONTIER_CS_EVAL_ARM=p50 FRONTIER_CS_EVAL_ID=<same id> \
      uv run --with modal modal run agentic_rl/launch/modal_train.py::post_process_data
...
Bootstrap hooks use the same file:
    ...
    uv run --with modal modal run agentic_rl/launch/modal_train.py::convert_hf_to_megatron_checkpoint
"""
...
from agentic_rl.launch.launch_config import (
    CHECKPOINTS_PATH,
    DATA_PATH,
    HF_CACHE_PATH,
    SLIME_ROOT,
...
# Locally this file sits at <repo>/agentic_rl/launch/modal_train.py and REPO_ROOT
