...
# Exclude VCS/build/cache junk; HF also rejects any path under a ".cache/" dir.
_COPY_IGNORE = shutil.ignore_patterns(".git", "__pycache__", ".DS_Store", ".venv", "node_modules", ".cache")
...
def convert(
    ...
    *,
    ...
    """Copy task trees + write train/eval JSONL. Returns (n_train, n_eval, skipped).

    ``key`` is the dataset key: it prefixes ``task_path`` (``<key>/tasks/<id>``) and
    is the ``/data/<key>/`` subdir the published repo lands in. Both ``train.jsonl``
    and ``eval.jsonl`` index the ONE shared ``tasks/`` tree by disjoint instance ids;
    ``eval_n`` rows are held out (``eval_only`` -> all rows to eval.jsonl, no train).
    """
    ...
    for task_dir in task_dirs:
        ...
        row["metadata"]["task_path"] = f"{key}/tasks/{instance_id}"
        rows.append(row)

    n_train, n_eval = _write_splits(rows, out_dir, eval_n=eval_n, seed=seed, eval_only=eval_only)
    logger.info("[harbor2slime] wrote train=%d eval=%d -> %s (skipped %d)", n_train, n_eval, out_dir, skipped)
    return n_train, n_eval, skipped


def _write_splits(rows: list[dict[str, Any]], out_dir: Path, *, eval_n: int, seed: int, eval_only: bool) -> tuple[int, int]:
    """Write train.jsonl / eval.jsonl (disjoint). ``eval_only`` -> all to eval.jsonl."""

    def _w(name: str, rs: list[dict[str, Any]]) -> None:
    ...
    _w("eval.jsonl", rows[:eval_n])
    _w("train.jsonl", rows[eval_n:])
    return len(rows) - eval_n, eval_n
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--key", help="dataset key = task_path prefix + /data subdir (default: dataset or tasks-dir name)")
    ...
    if args.registry:
        ...
        key = args.key or args.dataset
        dataset = args.dataset
    else:
        task_dirs = _discover_task_dirs(args.tasks_dir)
        key = args.key or args.tasks_dir.name
        dataset = args.tasks_dir.name
    if not task_dirs:
        raise SystemExit("no task dirs found")

    n_train, n_eval, skipped = convert(
        task_dirs, args.out_dir, key=key, dataset=dataset,
        eval_n=args.eval_n, seed=args.seed, eval_only=args.eval_only, limit=args.limit,
    )
    out_dir = args.out_dir.resolve()
    print(f"converted train={n_train} eval={n_eval} ({skipped} skipped) -> {out_dir} (task_path prefix '{key}')")
    print("next: publish out-dir to HF with path_in_repo=" + key + " (see convert2slime/README.md).")
