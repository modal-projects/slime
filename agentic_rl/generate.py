...
def _run_episode(task: dict, tokenizer, sampling_params, router_url: str, limits: dict, session_id: str, abort_check=None) -> tuple[float, RecordingModel, dict]:
...
async def generate(args, sample: Sample, sampling_params, evaluation: bool = False):
    ...
    if state.aborted:
        return _recycle_or_drop(args, sample, state.tokenizer, sample.prompt if isinstance(sample.prompt, str) else "", "StateAborted")
    ...
    try:
        ...
        logger.warning("episode exceeded hard cap %ds (instance=%s); recycling/dropping", hard_cap, task.get("instance_id"))
    ...
    if model.aborted:  # weight update aborted a turn mid-flight → recycle (async) / drop (sync)
        return _recycle_or_drop(args, sample, state.tokenizer, task.get("problem_statement", ""), "WeightUpdateAborted")
