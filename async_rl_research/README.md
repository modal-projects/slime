...
pluggable **envs**; the **harbor** env runs any harbor-format dataset (SWE-Gym,
SWE-bench-Pro, USACO, ...) with in-place `test.sh` verification, multi-step aware.
...
| `environment/base.py` | `RolloutEnv` contract (row schema, sandbox lifecycle, grading) + env registry; rows pick their env via `metadata.task_type` (absent -> `harbor`) |
| `environment/harbor.py` | Harbor env: Dockerfile boot, step loop, in-place verify (+ oracle-check CLI) |
| `environment/convert2slime/` | Dataset converters, paired with their env by filename (see `data/README.md`) |
