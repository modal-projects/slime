...
``/logs/verifier``); see ``conform_output_paths``.
    ...
    python -m async_rl_research.environment.convert2slime.openthoughts_agent \
...
from async_rl_research.environment.convert2slime import harbor
...
def materialize(
    ...
    *,
    ...
    try:
        return harbor.convert(task_dirs, out_dir, name=name, dataset=DATASET_NAME)
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"  python -m async_rl_research.environment.harbor {jsonl} --task-root {out_dir} --limit 3   # oracle check")
