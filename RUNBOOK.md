...
> **All-turns snapshot store — LANDED 2026-08-27** (design doc: the "All-Turns Snapshot Store" artifact; verification: `tests/test_agent/test_allturns_verification.py` + `agentic_rl/profiles/retro_snapshot_bench/`). `RETRO_CAPTURE_MODE=all_turns` stages EVERY post-tool boundary in-sandbox (`retro/turns.py`: rsync `--checksum --link-dest`, ~75 ms/turn measured; `--checksum` is load-bearing — the rsync quick-check silently staled same-size edits), packs ONE tarball artifact per admitted trajectory (~11 MiB / 75 turns measured; raw dir snapshots expand hardlinks 23x — never snapshot the staging dir raw), stores ONE final checkpoint as a volume blob (`retro/blobs.py` — manifest rows drop ~0.3–1.5 MB → ~KBs, winner mode included), and derives the turn-t checkpoint at restore by digest-verified truncation (`derive_checkpoint`; chain resets and in-place history mutation are refused, never silently replayed). Branch points become lease-time policies (`retro/selection.py`: `capture_default` parity / `target_fraction` / `hindsight_gain` via `RETRO_LEASE_POLICY`), and `ReplayPool.gc_aged` adds the age-out sweep (`RETRO_GC_MAX_AGE`, floored to the lease window) the bench showed is mandatory at this scale. Parity spine: `winner` mode is byte-identical to before; `all_turns` with `capture_default` reproduces the same branch points on the new store. Overhead A/B arm: `launch/arms/frontiercs_retro0_allturns.sh` vs `frontiercs_retro0_s4.sh`.
